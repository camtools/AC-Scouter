...
function showPage(pageId) {
    document.querySelectorAll('.page').forEach(p => {
        ...
        if (pageId === 'dop-page') {
        if (navigator.vibrate) navigator.vibrate(10); // "Sveglia" le vibrazioni
    }
    });
...
/* --- AGGIUNGI QUESTO SUBITO DOPO cineFocals --- */
const cineFocals = [16, 18, 20, 24, 28, 32, 40, 50, 65, 85, 100, 120, 135, 150, 180, 200, 300];
let lastSnappedFocal = null; 

// Test immediato: vibra quando entri nella pagina D.O.P.
function testHaptic() {
    if (navigator.vibrate) navigator.vibrate(10);
}

function updateFromRange() {
    const range = document.getElementById('focal-range');
    let val = parseInt(range.value);
    let currentSnap = null;

    // Logica di attrazione (Magnet)
    for (let f of cineFocals) {
        if (Math.abs(val - f) < 5) {
            val = f;
            range.value = f;
            currentSnap = f;
    ...
    // Feedback aptico: se abbiamo "agganciato" un magnete diverso dall'ultimo
    if (currentSnap !== null && currentSnap !== lastSnappedFocal) {
        if (navigator.vibrate) {
            navigator.vibrate(15); // Un colpo secco
            console.log("Haptic Click: " + currentSnap + "mm");
        }
        lastSnappedFocal = currentSnap;
    } 
    // Se usciamo dalla zona magnetica, resettiamo per il prossimo aggancio
    else if (currentSnap === null) {
        lastSnappedFocal = null;
    }
