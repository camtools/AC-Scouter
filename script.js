...
// MANCAVA QUESTA RIGA: La "memoria" per il feedback aptico
...
function updateFromRange() {
    ...
    // Controlliamo le ottiche nell'array cineFocals 
    for (let f of cineFocals) {
        if (Math.abs(val - f) < 5) {
            ...
            // Il cuore del feedback aptico: vibra SOLO se è una focale diversa dalla precedente
            if (lastSnappedFocal !== f) {
    ...
    // Se usciamo dalle zone "magnetiche", resettiamo la memoria
    if (!isSnapped) {
